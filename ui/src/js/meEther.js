...
const meEther = module.exports = {
	...
	meEther.MSContractInstance.vendorAccounts(addr, (err, resultObj) => {
	    ...
	    if (!err) {
		//result = { true, 0 }
		const keys = [ 'deliveriesApproved', 'deliveriesRejected', 'region', 'ratingSum', 'responseTimeSum', 'lastActivity', 'activeFlag' ];
	...
	meEther.MEContractInstance.escrowIds(acctAddr, idx, (err, resultObj) => {
		...
		const resultArray = Array.from(resultObj);
		const escrowKeys = [ 'isClosed', 'partnerAddr', 'vendorAddr', 'customerAddr', 'productId', 'vendorBalance', 'customerBalance', 'openDate', 'deliveryDate' ];
		for (let i = 0; i < resultArray.length; ++i)
