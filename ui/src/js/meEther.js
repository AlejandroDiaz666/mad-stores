...
const meEther = module.exports = {
    ...
    //ropsten
    //MadStores
    MS_CONTRACT_ADDR: '0xaF20fbD525586c0AF87C610801811aBF914F8F2B',
	...
	if (!meEther.purchaseDeclineABI)
	    meEther.purchaseDeclineABI = ethabi.methodID('purchaseDecline', [ 'uint256', 'uint256', 'uint256', 'bytes' ]).toString('hex');
	...
	meEther.MEContractInstance.escrowIds(acctAddr, idx, (err, resultObj) => {
		...
		const escrowInfo = {};
		const keys = [ 'isClosed', 'isApproved', 'isBurned', 'partnerAddr', 'vendorAddr', 'customerAddr', 'productId', 'vendorBalance',
			       'customerBalance', 'deliveryDate', 'createXactId', 'modifyXactId', 'approveCancelXactId', 'releaseBurnXactId' ];
		const resultArray = Array.from(resultObj);
		for (let i = 0; i < resultArray.length; ++i)
