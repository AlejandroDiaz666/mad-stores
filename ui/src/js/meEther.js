...
const meEther = module.exports = {
	...
	if (network.indexOf('Mainnet') >= 0) {
	    ...
	    mtEther.firstBlock = 5478364;
	}
	if (!mtEther.firstBlock)
	    err = network + ' is not a supported network';
	console.log('setNetwork: first block = ' + mtEther.firstBlock);
	return(err);
    },
    ...
    getCertainProducts: function(vendorAddr, categoryBN, regionBN, minPriceBN, maxPriceBN, minDeliveriesBN, minRatingBN, onlyAvailable, productStartIdxBN, maxResults, cb) {
	if (!meEther.MSContractInstance)
	    initMSContractInstance();
	meEther.MSContractInstance.getCertainProducts(vendorAddr, common.BNToHex256(categoryBN), common.BNToHex256(regionBN),
						      ...
						      common.BNToHex256(minDeliveriesBN), common.BNToHex256(minRatingBN),
    ...
    //cb(err, lastSearchIdx, productIDs[])
    getVendorProducts: function (vendorAddr, categoryBN, regionBN, minPriceBN, maxPriceBN, minDeliveriesBN, minRatingBN, onlyAvailable, productStartIdxBN, maxResults, cb) {
	if (!meEther.MSContractInstance)
	    initMSContractInstance();
    ...
    //cb(err, lastSearchIdx, productIDs[])
    getCategoryProducts: function (vendorAddr, categoryBN, regionBN, minPriceBN, maxPriceBN, minDeliveriesBN, minRatingBN, onlyAvailable, productStartIdxBN, maxResults, cb) {
    ...
    //cb(err, lastSearchIdx, productIDs[])
    getRegionProducts: function (vendorAddr, categoryBN, regionBN, minPriceBN, maxPriceBN, minDeliveriesBN, minRatingBN, onlyAvailable, productStartIdxBN, maxResults, cb) {
	if (!meEther.MSContractInstance)
	    initMSContractInstance();
	...
	meEther.MEContractInstance.escrowIds(acctAddr, idx, (err, resultObj) => {
		...
		const resultArray = Array.from(resultObj);
		const escrowKeys = [ 'isClosed', 'partnerAddr', 'vendorAddr', 'customerAddr', 'productId', 'vendorBalance', 'customerBalance', 'deliveryDate' ];
		for (let i = 0; i < resultArray.length; ++i)
