...
const meEther = module.exports = {
    ...
    //Turms Token and Dai contracts use ERC20 iface
    TT_CONTRACT_ADDR: null,
    DAI_CONTRACT_ADDR: null,
	...
	if (network.indexOf('Mainnet') >= 0) {
	    ...
	    meEther.TT_CONTRACT_ADDR = meEther.mainnet_tt_contract_addr;
	    meEther.DAI_CONTRACT_ADDR = meEther.mainnet_dai_contract_addr;
	    ...
	    meEther.TT_CONTRACT_ADDR = meEther.ropsten_tt_contract_addr;
	    meEther.DAI_CONTRACT_ADDR = meEther.ropsten_dai_contract_addr;
    ...
    //
    // get Turms Token balance
    // cb(err, balanceBN)
    //
    getTTBalance: function(acctAddr, cb) {
	if (!meEther.TTContractInstance)
	    initTTContractInstance();
	meEther.TTContractInstance.balanceOf(acctAddr, (err, resultObj) => {
	    console.log('getTTBalance: acctAddr = ' + acctAddr + ', err = ' + err + ', result = ' + resultObj.toString());
	    if (!!err) {
		cb(err, null);
		return;
	    }
...
function initTTContractInstance() {
    const ABIArray = JSON.parse(meEther.ERC20_ABI);
    const ttContract = common.web3.eth.contract(ABIArray);
    console.log('meEther.initTTContractInstance: contract addr: ' + meEther.TT_CONTRACT_ADDR);
    meEther.TTContractInstance = ttContract.at(meEther.TT_CONTRACT_ADDR);
}
