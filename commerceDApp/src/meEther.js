...
var meEther = module.exports = {
    ...
    //cb(err, txid)
    registerVendor: function(web3, serviceRegionBN, descHex, imageHex, cb) {
	var abiRegisterVendorFcn = meEther.abiEncodeRegisterVendor();
	var abiParms = meEther.abiEncodeRegisterVendorParms(serviceRegionBN, descHex, imageHex);
	...
	if (!meEther.registerVendorABI)
	    meEther.registerVendorABI = ethabi.methodID('registerVendor', [ 'uint256', 'bytes', 'bytes' ]).toString('hex');
	return(meEther.registerVendorABI);
    },

    abiEncodeRegisterVendorParms: function(serviceRegionBN, descHex, imageHex) {
	var descBytes = common.hexToBytes(descHex);
	var imageBytes = common.hexToBytes(imageHex);
	encoded = ethabi.rawEncode([ 'uint256', 'bytes', 'bytes' ],
				   [ serviceRegionBN, descBytes, imageBytes ] ).toString('hex');
	return(encoded);
    },
